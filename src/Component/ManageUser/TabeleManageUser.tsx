...
export default function TabeleManageUser() {
  ...
  return (
    <div>
      <div dir="rtl" className="relative overflow-x-auto shadow-md sm:rounded-lg w-full">
            ...
            {users.map((user) => (
              <tr key={user.id} className="bg-white text-center border-b hover:bg-gray-50">
                ...
                <td className="md:px-6 px-3 py-4">
                  {user.avatar ? (
                    <img className='m-2 w-14 h-14 mx-auto border-2 bg-gray-300 border-gray-400 rounded-full' src={`https://195.191.45.56:5155/uploads/${user.avatar}`} />
                  ) : (
                    <img className='m-2 w-14 h-14 mx-auto border-2 bg-gray-300 border-gray-400 rounded-full' src={noneAvatar} />
                  )}
                </td>
                ...
                <td className="md:px-6 px-3 py-4">
                  <div className="flex items-center justify-center">
                    ...
                    <span onClick={() => { navigate(`/Admin/ManageUser/edit/${user.id}`); }} className="text-green-600 cursor-pointer text-2xl border rounded-full p-2 border-green-400 hover:text-white hover:bg-green-400 mx-2 duration-200">
                      <CiEdit />
                    </span>
      ...
      <div className="flex justify-center mt-4">
        ...
        <span className="px-4 py-2 text-gray-700">{pageination} / {totalPages}</span>
        <button 
